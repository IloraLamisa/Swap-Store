// src/screens/FurnitureScreen.js
...
import {
  ...
  getFurnitureOnSaleProducts,
} from '../../assets/data/Products';

export default function FurnitureScreen({ navigation }) {
  const allFurniture = useMemo(() => getFurnitureProducts(), []);
  const onSaleFurniture = useMemo(() => getFurnitureOnSaleProducts(), []);
  ...
  return (
    <SafeAreaView style={styles.safe}>
      ...
      >
        ...
        {/* On Sale Furniture Section */}
        <Text style={styles.sectionTitle}>On Sale 🔥</Text>
        <FlatList
          data={onSaleFurniture}
          keyExtractor={(item) => item.id}
          numColumns={2}
          scrollEnabled={false}
          columnWrapperStyle={styles.row}
