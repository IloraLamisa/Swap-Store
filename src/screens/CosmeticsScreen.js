// src/screens/CosmeticsScreen.js
...
import {
  getCosmeticsOnSaleProducts,
  getCosmeticsProducts
} from '../../assets/data/Products';

export default function CosmeticsScreen({ navigation }) {
  const allCosmetics = useMemo(() => getCosmeticsProducts(), []);
  const onSaleCosmetics = useMemo(() => getCosmeticsOnSaleProducts(), []);
  ...
  return (
    <SafeAreaView style={styles.safe}>
      ...
      >
        ...
        {/* On Sale Cosmetics Section */}
        <Text style={styles.sectionTitle}>On Sale 🔥</Text>
        <FlatList
          data={onSaleCosmetics}
          keyExtractor={(item) => item.id}
          numColumns={2}
          scrollEnabled={false}
          columnWrapperStyle={styles.row}
